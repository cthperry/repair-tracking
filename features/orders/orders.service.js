...
getSummaryForRepair(repairId) {
  ...
  getAll() {
    // remove() 採軟刪除（isDeleted=true），列表需排除已刪除項目。
    return (this.orders || [])
  ...
  async createFromQuote(quoteId, options = {}) {
    ...
    const quote = _svc('QuoteService')?.get?.(quoteId);
    ...
    const repair = _svc('RepairService')?.get?.(quote.repairId) || null;
    ...
    try {
      ...
      if (rid && _svc('RepairPartsService')?.update) {
        const list = _svc('RepairPartsService').getForRepair(rid);
        for (const p of list) {
          if (!p || p.isDeleted) continue;
          if (p.quoteId === quote.id && (p.status === '已報價' || p.status === '需求提出' || !p.status)) {
            await _svc('RepairPartsService').update(rid, p.id, { status: '已下單', orderId: order.id });
          }
        }
      }
    } catch (e) {
...
async createFromRepair(repair) {
  ...
  let quote = null;
  try {
    if (_svc('QuoteService')) {
      if (typeof _svc('QuoteService').getLatestForRepair === 'function') {
        quote = _svc('QuoteService').getLatestForRepair(repair.id);
      } else if (typeof _svc('QuoteService').getSummaryForRepair === 'function') {
        quote = _svc('QuoteService').getSummaryForRepair(repair.id)?.latest || null;
      } else if (typeof _svc('QuoteService').getForRepair === 'function') {
        const qs = await _svc('QuoteService').getForRepair(repair.id);
        quote = (window.Utils?.pickLatest) ? window.Utils.pickLatest(qs) : (qs && qs[0]) || null;
      }
    }

    if (!quote && _svc('QuoteService') && typeof _svc('QuoteService').createFromRepair === 'function') {
      quote = await _svc('QuoteService').createFromRepair(repair);
    }
  } catch (e) {
      ...
      if (repairId && _svc('RepairPartsService')?.getForRepair && _svc('RepairPartsService')?.update) {
        ...
        for (const p of list) {
          ...
          if (p.orderId === oid && (p.status === '已下單' || !p.status)) {
            await _svc('RepairPartsService').update(repairId, p.id, { status: '已報價', orderId: null });
          }
        }
      }
    } catch (e) {
