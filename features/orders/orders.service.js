    ...
    try {
      ...
      if (raw) {
        ...
        this.orders = Array.isArray(data)
          ? data.map(OrderModel.normalize).filter(Boolean)
          : [];
      } else {
        this.orders = [];
      }
...
async upsert(order) {
    ...
    const prevOrders = Array.isArray(this.orders) ? this.orders.slice() : [];
