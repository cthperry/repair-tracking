  ...
  renderCreateFromQuoteModal() {
    return `
      <div class="modal-dialog">
        ...
        <form class="modal-body enterprise-form order-create-form" data-action="orders.handleCreateFromQuote">
          <div class="form-section">
            <div class="form-section-head"><h4 class="form-section-title">選擇報價</h4><p class="form-section-desc">由已核准報價建立訂單，維持商務流程順序清楚。</p></div>
    ...
    return `
      <div class="modal-dialog modal-xlarge order-detail-modal">
        ...
        <form class="modal-body enterprise-form order-form" id="order-detail-form-${idSafe}" data-action="orders.handleSaveOrder">
          ...
          <div class="form-section">
            <div class="form-section-head"><h4 class="form-section-title">狀態與供應商</h4><p class="form-section-desc">狀態、幣別、供應商與日期欄位固定同區，避免採購資訊散落。</p></div>
            <div class="form-grid">
              <div class="form-group">
          ...
          <div class="form-section">
            <div class="order-items-head">
              <div class="form-section-head"><h4 class="form-section-title">項目</h4><p class="form-section-desc">項目表單沿用 Quote 規則，確保從報價轉訂單後仍維持相同閱讀習慣。</p></div>
          ...
          <div class="form-section">
            <div class="form-section-head"><h4 class="form-section-title">備註</h4><p class="form-section-desc">用於補充交期、採購備註與收貨說明，不與正式品項混寫。</p></div>
...
Object.assign(OrdersUI, {
  ...
  _validateItemsForm(form, count, moduleLabel) {
    ...
    if (!items.length) {
      const firstControl = form.querySelector('[name="name_0"]') || form.querySelector('[data-action="orders.addItem"]');
      return this._failForm(form, firstControl, '請至少輸入一筆訂單項目', {
        summaryMessage: '訂單至少需要一筆有效項目'
      });
    }
  ...
  async handleCreateFromQuote(event) {
    ...
    if (window.FormValidate && !window.FormValidate.validateForm(form, { summaryMessage: '請先選擇報價後再建立訂單' })) {
      return;
    }
    const data = Object.fromEntries(new FormData(event.target).entries());
    const qid = (data.quoteId || '').trim();
    if (!qid) {
      const quoteEl = form.querySelector('[name="quoteId"]');
      this._failForm(form, quoteEl, '請選擇報價', { summaryMessage: '請先選擇報價後再建立訂單' });
      return;
    }
    try {
  ...
  async handleSaveOrder(event) {
    ...
    try {
      ...
      const items = this._validateItemsForm(form, count, '訂單');
      ...
      // 讓觸控事件完整結束後再關閉 modal，降低儲存後穿透到下層按鈕的風險。
      await new Promise((resolve) => setTimeout(resolve, 80));
      OrdersUI.closeModal();
      if (window.UI && typeof window.UI.toast === 'function') window.UI.toast('訂單已儲存', { type: 'success' });
    } catch (e) {
      console.error(e);
      ...
      try { delete form.dataset.saving; } catch (_) { form.dataset.saving = ''; }
    }
  },
  ...
  try {
    ...
    const inOrders = (t) => {
      try {
        ...
        return !!t.closest('.orders-module, #orders-modal, #orders-modal-content, .order-detail-modal, .create-order-modal');
      } catch (_) {
        return false;
      }
    };
