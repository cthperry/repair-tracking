...
class RepairModel {
   ...
   * - notOrdered: { stageCode: 'quote_pending'|'procurement'|'reviewing'|'budget_review'|'on_hold'|'other'|null, reasonCode: 'price'|'budget'|'internal'|'spec'|'other'|null, note: string|null }
  ...
  static normalizeBilling(repair, before = null) {
    ...
    let stageCode = (n0?.stageCode ?? null);
    stageCode = (stageCode === null || stageCode === undefined) ? null : String(stageCode);
    if (stageCode) {
      const s = stageCode.toLowerCase();
      if (['quote_pending','procurement','reviewing','budget_review','on_hold','other'].includes(s)) stageCode = s;
      else stageCode = 'other';
    }
    ...
    if (reasonCode) {
      ...
      if (['price','budget','internal','spec','other'].includes(s)) reasonCode = s;
    ...
    if (chargeable !== true) {
      ...
      if (orderStatus === 'ordered') {
        ...
        // keep stage/reason/note (nullable)
      } else {
        orderStatus = null;
        stageCode = null;
        reasonCode = null;
  ...
  static create(data = {}) {
    ...
    const base = {
      ...
      tags: data.tags || [],
      // SOP 關聯（SOP Hub / 知識庫）
      // 格式：[{ sopId: string, version?: number }]
      sopRefs: Array.isArray(data.sopRefs) ? data.sopRefs : [],
    ...
    const withBilling = this.normalizeBilling({
      ...
      billing: (data.billing && typeof data.billing === 'object') ? data.billing : {
        ...
        notOrdered: {
          stageCode: (data.notOrderedStage !== undefined) ? data.notOrderedStage : null,
...
static normalizeStatusProgress(repair) {
  ...
  // 規則：
  // 1) 100% 一律視為「已完成」（needParts 可保留作為標記）
